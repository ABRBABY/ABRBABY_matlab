function bt_xlswrite(m,header,colnames,filename,sheetname);
% bt_xlswrite     Easily create an Excel spreadsheet from MATLAB
%
%  bt_xlswrite(m,header,colnames,filename) creates a Microsoft Excel spreadsheet using
...
% 
% modified 11/22/05 eeskoe
% The original incarnation of xlswrite could only generate xls file in the
% current working directory. The function has been modified to read the
% path information from the filename and save to the current directory only
...
%     invoke(op, 'SaveAs', [pwd filesep filename]);
     ...
     if isempty(pathstr)  %if loop added November 21, 2005 (eeskoe)
        pathstr = pwd;
