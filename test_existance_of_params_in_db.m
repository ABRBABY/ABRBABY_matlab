...
for jj=1:length(subjects) 
    ...
    if isempty(RFE_num)
        [does_exist, count(jj)] = check_exist_set_params(subjects{jj}, strcat(subjects{jj},'_',OPTIONS.analysis,'*', suffix,'*'), OPTIONS) ; 
    else
        [does_exist, count(jj)] = check_exist_set_params(subjects{jj}, strcat(subjects{jj},'_',OPTIONS.analysis,'*', RFE_num, '*',suffix,'*'), OPTIONS) ; 
    end
