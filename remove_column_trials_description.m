...
for jj=1:length(subjects)
    ...
    %Remove automatic rejection ERP column if selected
    if remove_autorej_erp==1 && sum(contains(T1.Properties.VariableNames,'ERP_autorej'))>0
        autorej_erp_variableIndex = find(contains(T1.Properties.VariableNames,'ERP_autorej'));
        T1(:,autorej_erp_variableIndex) = [] ;
    end
    %Remove mautomatic rejection FFR column if selected
